...
function update_clock(delta){
    if(scoreboard_state.time.is_game_clock_running){
        ...
        scoreboard_state.time.game_clock_current_time = Math.max(scoreboard_state.time.game_clock_current_time, 0)
    ...
    if(scoreboard_state.time.is_play_clock_running && scoreboard_state.time.is_game_clock_running){
        scoreboard_state.time.play_clock_current_time -= delta
        scoreboard_state.time.play_clock_current_time = Math.max(scoreboard_state.time.play_clock_current_time, 0)
            ...
            if(ws.auth.is_authenticated){
                if(action.type == "set_team_score"){
                    if(action.payload.team === "home"){
                        ...
                        scoreboard_state.team_away.score = clamp(action.payload.score, 0, 9999)
                    }
                    broadcast({
                        type: "sync:score",
                        payload: {home_score: scoreboard_state.team_home.score, away_score: scoreboard_state.team_away.score}
                    })
                }
                ...
                if(action.type == "set_team_color"){
                    ...
                    broadcast({
                        type: "sync:color",
                        payload: {home_color: scoreboard_state.team_home.color, away_color: scoreboard_state.team_away.color}
                    })
                }
                if(action.type == "add_team_score"){
                    let target_team_state = action.payload.team === "home" ? scoreboard_state.team_home : scoreboard_state.team_away
                    target_team_state.score += action.payload.amount
                    target_team_state.score = clamp(target_team_state.score, 0, 9999)
                    broadcast({
                        type: "add_team_score",
                        payload: {team: action.payload.team, animation: action.payload.animation, animation_type: action.payload.animation_type, amount: action.payload.amount, previous_score: target_team_state.score - action.payload.amount, new_score: target_team_state.score}
                    })
                }
                if(action.type == "set_clock_state"){
                    if(action.payload.clock == "game"){
                        scoreboard_state.time.is_game_clock_running = action.payload.new_state;
                    }
                    if(action.payload.clock == "play"){
                        scoreboard_state.time.is_play_clock_running = action.payload.new_state;
                    }
                    broadcast({
                        type:"sync:time",
                        payload:scoreboard_state.time
                ...
                if(action.type == "add_clock_time"){
                    if(action.payload.clock == "game"){
                        scoreboard_state.time.game_clock_current_time = Math.max(scoreboard_state.time.game_clock_current_time + action.payload.amount, 0)
                    }
                    if(action.payload.clock == "play"){
                        scoreboard_state.time.play_clock_current_time = clamp(scoreboard_state.time.play_clock_current_time + action.payload.amount, 0, 60 * 1000)
                    }
                    broadcast({
                        type:"sync:time",
                        payload:scoreboard_state.time
                ...
                if(action.type == "set_clock_time"){
                    if(action.payload.clock == "game"){
                        scoreboard_state.time.game_clock_current_time = Math.max(action.payload.time, 0)
                        
                    }
                    if(action.payload.clock == "play"){
                        scoreboard_state.time.play_clock_current_time = clamp(action.payload.time, 0, 60 * 1000)
                    }
                    broadcast({
                        type:"sync:time",
                        payload:scoreboard_state.time
                ...
                if(action.type == "set_down"){
                    ...
                    broadcast({
                        type: "sync:down",
                        payload: { down: scoreboard_state.down, distance: scoreboard_state.distance }
                    });
                }
                ...
                if(action.type == "add_quarter"){
                    ...
                    scoreboard_state.quarter = clamp(scoreboard_state.quarter, 0, 5);
                    broadcast({
                        type:"sync:quarter",
                        payload:{quarter:scoreboard_state.quarter}
                    });
                ...
                if(action.type == "set_team_timeouts"){
                    if(action.payload.team === "home"){
                        scoreboard_state.team_home.timeouts_remaining = clamp(action.payload.timeouts, 0, 3)
                    } else if(action.payload.team === "away"){
                        scoreboard_state.team_away.timeouts_remaining = clamp(action.payload.timeouts, 0, 3)
                    }
                    broadcast({
                        type: "sync:timeouts",
                        payload: {home_timeouts: scoreboard_state.team_home.timeouts_remaining, away_timeouts: scoreboard_state.team_away.timeouts_remaining}
                    })
                }
                if(action.type == "add_team_timeouts"){
                    let target_team_state = action.payload.team === "home" ? scoreboard_state.team_home : scoreboard_state.team_away
                    target_team_state.timeouts_remaining += action.payload.amount
                    target_team_state.timeouts_remaining = clamp(target_team_state.timeouts_remaining, 0, 3)
                    broadcast({
                        type: "sync:timeouts",
                        payload: {home_timeouts: scoreboard_state.team_home.timeouts_remaining, away_timeouts: scoreboard_state.team_away.timeouts_remaining}
                    })
